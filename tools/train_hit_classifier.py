...
def main():
    ...
    # Split (Group-based to prevent leakage if clip_id is available)
    if "clip_id" in df.columns and df["clip_id"].nunique() > 1:
        from sklearn.model_selection import GroupShuffleSplit
        gss = GroupShuffleSplit(n_splits=1, test_size=0.2, random_state=42)
        train_idx, test_idx = next(gss.split(X, y, groups=df["clip_id"]))
    ...
    print(f"ความแม่นยำ (Accuracy on 20% test set): {acc*100:.2f}%")
    print("\nรายงานผลการทดสอบ:")
    ...
    # Save model - Fit on all data for final export
    print("\nกำลังเทรนโมเดล Final ด้วยข้อมูลทั้งหมดก่อนบันทึก...")
    clf_final = RandomForestClassifier(n_estimators=100, max_depth=5, random_state=42, class_weight="balanced")
    clf_final.fit(X, y)
    with open(model_file, "wb") as f:
        pickle.dump((clf_final, feature_cols), f)
