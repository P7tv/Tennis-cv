...
def main():
    ...
    for tr, te in logo.split(X, y, df["person"]):
        ...
        train_data["is_hit"] = y.iloc[tr].values
        
        predictor = TabularPredictor(label="is_hit", verbosity=0).fit(
            train_data, presets='medium_quality', time_limit=30)
        
        if len(te_real):
            test_data = pd.DataFrame(X.iloc[te_real])
            df.iloc[te_real, df.columns.get_loc("oof_prob")] = \
                predictor.predict_proba(test_data).iloc[:, 1].values
    ...
    train_data_full["is_hit"] = y.values
    ...
    print("\nความสำคัญของฟีเจอร์:")
    try:
        imp = predictor.feature_importance(train_data_full)
        print(imp)
    except Exception as e:
        print(f"Cannot compute feature importance: {e}")

    if args.save:
        with open(Path(ag_out_dir) / "feature_cols.json", "w") as f:
            json.dump(FEATURE_COLS, f)
        print(f"\nบันทึกโมเดล -> {ag_out_dir}")
    else:
    ...
    if args.lopo_out:
        ...
        for person in sorted(df_all["person"].unique()):
            ...
            train_data = pd.DataFrame(df_all.loc[tr, FEATURE_COLS].fillna(0))
            train_data["is_hit"] = df_all.loc[tr, "is_hit"].values
            
            p_out_dir = out_dir / f"{person}_ag"
            m = TabularPredictor(label="is_hit", path=str(p_out_dir), verbosity=0).fit(
                train_data, presets='medium_quality', time_limit=30)
            
            with open(p_out_dir / "feature_cols.json", "w") as f:
                json.dump(FEATURE_COLS, f)
            print(f"  LOPO model (ไม่มี {person}): "
                  f"{int(tr.sum())} แถว -> {p_out_dir}")
